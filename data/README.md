...
| `credentials.json` | (Optionnel). Fichier manuel dans lequel vous pouvez configurer des identifiants valides (email/mot de passe) pour forcer le bot à attaquer une cible sans utiliser la fonction "Inscription Automatique". | Si l'inscription publique est désactivée sur la cible, le bot ne pourra pas réaliser l'audit s'il n'a pas ce fichier. |
