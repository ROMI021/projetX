...
| `spider.js` | Moteur DAST (Dynamic Application Security Testing). Lance un navigateur invisible (*Puppeteer Stealth*) pour extraire dynamiquement les routes cachées des cibles. **Désormais capable d'intercepter les requêtes XHR, Fetch et les soumissions de formulaires classiques (Document) tout en filtrant les trackers analytiques tiers.** | Incapacité d'auditer des cibles réelles ne possédant pas de documentation API ouverte. |
| `store.js` | Gère l'état global de l'application (Variables en mémoire RAM) et la sauvegarde sur disque (`state.json`). Contient aussi `broadcastEvent` pour envoyer les logs en temps réel (SSE) à l'UI. | Perte totale de la mémoire du programme. Plantage généralisé des flux SSE. |
| `utils.js` | Fonctions utilitaires de base (Requêtes HTTP, cryptographie). Contient la **logique de Probe (Options)** pour découvrir l'architecture cible en écartant les faux positifs (comme les erreurs 404). | Dépendance critique pour tous les autres modules. Plantage garanti. |
