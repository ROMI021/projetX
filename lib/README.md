...
| `spider.js` | Moteur DAST. Lance un navigateur invisible (*Puppeteer Stealth*). **Simule désormais de véritables frappes de touches humaines (délais aléatoires, clics lents) pour déjouer les WAF.** Il injecte les identifiants réels (`credentials.json`), priorise l'onglet Email et se met en **pause asynchrone (max 3 min)** lorsqu'un code OTP est requis. | Incapacité d'auditer des cibles réelles nécessitant une double authentification ou bloquant les saisies DOM instantanées. |
...
| `utils.js` | Fonctions utilitaires de base (Requêtes HTTP, cryptographie). **Charge désormais automatiquement les identifiants depuis le disque (`loadTargetCredentials`)** pour les fournir au Spider. | Dépendance critique pour tous les autres modules. Plantage garanti. |
