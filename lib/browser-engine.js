...
import { generateResourceAutonomous } from './macro-engine.js';
...
export async function runHeavyAudit(targetBase, routes, credentials, targetEndpoint) {
    ...
    try {
        ...
        // 3.5 Génération Autonome de Ressource (Auto-Macro) si nécessaire
        ...
        if (cleanTargetEndpoint.includes(':id') || cleanTargetEndpoint.endsWith('N/A')) {
            const macroResult = await generateResourceAutonomous(pageA, targetBase, broadcastEvent);
            if (macroResult && macroResult.objectId) {
                finalObjectId = macroResult.objectId;
                
                // Si la macro a aussi découvert la vraie route (ex: /api/carts)
                if (macroResult.endpoint) {
                     cleanTargetEndpoint = macroResult.endpoint;
                     if (!cleanTargetEndpoint.endsWith(finalObjectId)) {
                         cleanTargetEndpoint = cleanTargetEndpoint.replace(/\/$/, '') + '/' + finalObjectId;
                     }
                } else {
                     // Remplacement classique dans la route générique
                     cleanTargetEndpoint = cleanTargetEndpoint.replace(':id', finalObjectId).replace('N/A', finalObjectId);
                }
                
                broadcastEvent({ origin: 'AUDIT', type: 'success', msg: `[MACRO] Cible dynamique mise à jour : ${cleanTargetEndpoint}` });
            }
        }
