...
async function submitLoginFormWithPoT(page, creds) {
    try {
        ...
        for (const btn of buttons) {
            ...
            if (text.includes('submit') || text.includes('login') || text.includes('connexion') || text.includes('send') || text.includes('se connecter')) {
                await btn.click().catch(() => {});
                submitted = true;
                broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[HEAVY ENGINE] Bouton submit cliqué: "${text}"` });
                break;
            }
        }
