...
async function authenticateInPage(page, targetBase, loginRoute, userCreds) {
    ...
    try {
        ...
        const loginUrl = new URL(loginRoute || '/login', targetBase).toString();
        await page.goto(loginUrl, { waitUntil: 'domcontentloaded', timeout: PUPPETEER_TIMEOUT_MS }).catch(() => {});

        broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[HEAVY ENGINE] Navigation vers ${loginRoute}...` });

        // Essai 1: Extraire le PoT du formulaire HTML et soumettre
        let result = await submitLoginFormWithPoT(page, userCreds);
        if (result.success) {
        ...
        result = await submitLoginAPI(page, targetBase, loginRoute, userCreds, captchaToken, proofOfTime);
        if (result.success) {
...
async function submitLoginFormWithPoT(page, creds) {
    try {
        ...
        for (const input of inputs) {
            ...
            // Skip hidden inputs sauf PoT
            if (inputType === 'hidden' && !inputName.includes('pot')) {
                continue;
            }

            // Remplir email
            if (inputType === 'email' || inputName.includes('email') || inputName.includes('user') || inputName.includes('login')) {
                await input.type(creds.email, { delay: 50 }).catch(() => {});
                filledCount++;
                broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[HEAVY ENGINE] Email rempli: ${creds.email}` });
            } 
            // Remplir password
            else if (inputType === 'password' || inputName.includes('pass')) {
                await input.type(creds.password, { delay: 50 }).catch(() => {});
                filledCount++;
        ...
        if (filledCount < 2) {
            broadcastEvent({ origin: 'AUDIT', type: 'warning', msg: `[HEAVY ENGINE] Seulement ${filledCount}/2 champs remplis` });
        ...
        for (const btn of buttons) {
            ...
            if (text.includes('submit') || text.includes('login') || text.includes('connexion') || text.includes('send')) {
                await btn.click().catch(() => {});
                submitted = true;
                broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[HEAVY ENGINE] Bouton submit cliqué` });
                break;
            }
        }
        ...
        if (!submitted) {
            ...
            broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[HEAVY ENGINE] Tentative Enter` });
        }

        // IMPORTANT: Attendre que le serveur réponde et définisse les cookies
        // Écouter les requêtes réseau pour savoir quand la réponse arrive
 ...
 */
async function executeFetchInPage(page, url, method = 'GET') {
    try {
