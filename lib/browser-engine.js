...
export async function runHeavyAudit(targetBase, routes, credentials, targetEndpoint) {
    ...
    try {
        ...
        if (cleanTargetEndpoint.includes(':id') || cleanTargetEndpoint.endsWith('N/A')) {
            const capturedId = await generateResourceAutonomous(pageA, targetBase, broadcastEvent);
            if (capturedId) {
                finalObjectId = capturedId;
                cleanTargetEndpoint = cleanTargetEndpoint.replace(':id', capturedId).replace('N/A', capturedId);
