...
import { runHeavyAudit } from './browser-engine.js';
...
function extractPossibleObjectIds(data, seen = new Set()) {
  ...
  const addCandidate = value => {
    ...
    if (/^[0-9]{1,7}$/.test(value)) {
      ids.add(value); // Accepter les identifiants numériques courts (ex: "1001")
    }
    else if (/^[A-Za-z0-9_-]{8,}$/.test(value)) {
      ids.add(value); // Accepter les UUIDs et les hash
    }
  };
  ...
  for (const [key, value] of Object.entries(data)) {
    if (typeof value === 'string' || typeof value === 'number') {
      ...
      if (/(?:^|_|\b)(?:id|invoice|order|user|customer|account|profile)\b/i.test(key)) {
        addCandidate(strValue);
      } else if (/^[A-Za-z0-9_-]{8,}$/.test(strValue) || /^[0-9]{1,7}$/.test(strValue)) {
        addCandidate(strValue);
      }
    }
...
async function postAuthCandidate(url, candidate, label = 'candidate', payloads = []) {
  ...
  // 3.5. Evasion: Permutations de clés d'identifiant pour tromper les WAFs et s'adapter aux APIs (email, username, identifier, login)
  const basePayloads = [candidate];
  if (candidate.email) {
      basePayloads.push({ username: candidate.email, password: candidate.password, clientEntropy: candidate.clientEntropy, potToken: candidate.potToken });
      basePayloads.push({ identifier: candidate.email, password: candidate.password, clientEntropy: candidate.clientEntropy, potToken: candidate.potToken });
      basePayloads.push({ login: candidate.email, password: candidate.password, clientEntropy: candidate.clientEntropy, potToken: candidate.potToken });
  }

  // 3.6 Evasion: Format API spécifiques (ex: Nuxt/Strapi enveloppent parfois dans un objet "data" et exigent des champs stricts)
  // IMPORTANT: On enlève clientEntropy et potToken pour éviter que la validation stricte de l'API (Joi/Zod) ne rejette la requête (400 Bad Request)
  const cleanPayloads = basePayloads.map(p => {
      const clean = { ...p };
      delete clean.clientEntropy;
      delete clean.potToken;
      return clean;
  });
  const wrappedPayloads = cleanPayloads.map(p => ({ data: { ...p, cart_id: null } }));
  basePayloads.push(...wrappedPayloads);

  let payloadQueue = [];
  if (payloads.length > 0) {
      for (const base of basePayloads) {
          payloadQueue.push(base);
          for (const overlay of payloads) {
              payloadQueue.push({ ...base, ...overlay });
          }
      }
  } else {
      payloadQueue = basePayloads;
  }
  ...
  for (const payload of payloadQueue) {
    for (const contentType of contentVariants) {
      ...
      // 4. Evasion: Empreinte Navigateur Stricte + Headers de contexte
...
async function discoverTarget(targetBase) {
  ...
  try {
    ...
    if (registerProbe.status >= 200 && registerProbe.status < 500 && registerProbe.status !== 404) {
      authModel = authModel === 'login-route-detected' ? 'auth-routes-detected' : 'registration-route-detected';
    ...
    if (loginProbe.status >= 200 && loginProbe.status < 500 && loginProbe.status !== 404) {
      authModel = authModel === 'registration-route-detected' ? 'auth-routes-detected' : 'login-route-detected';
  ...
  // Si l'analyse OpenAPI a échoué, on lance toujours le Spider pour trouver les vraies routes d'authentification 
  // (les requêtes OPTIONS peuvent être trompées par des WAFs renvoyant 403)
  if (!openApiCandidates.length) {
      const credentials = loadTargetCredentials(targetBase) || {};
      const spiderResult = await spiderTarget(targetBase, credentials);
