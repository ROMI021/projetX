...
export async function spiderTarget(targetBase, credentials = {}) {
  ...
  try {
           ...
           if (pathObj.hostname.includes(targetObj.hostname.replace('www.', ''))) {
             ...
             if (/login|connexion|signin|auth|customer|account|user/i.test(pathname) && method === 'POST') {
                 ...
                 try {
                     ...
                     if (postData) {
                         broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[DEBUG] Payload exact envoyé par le navigateur : ${postData.substring(0, 200)}` });
                     }
                 } catch(e) {}
             }
    ...
    const targetHost = new URL(targetBase).hostname.replace('www.', '');
    const authLinks = await page.$$eval('a', (anchors, host) => {
      return anchors.map(a => a.href).filter(href => {
         try {
             const u = new URL(href);
             if (!u.hostname.includes(host)) return false; // Ignorer les liens externes (Facebook, etc.)
             return /login|connexion|register|inscription|signup|signin|auth|account|user|profile|my-account/i.test(u.pathname);
         } catch(e) {
             return false;
         }
      });
    ...
    if (uniqueLinks.length === 0) {
        broadcastEvent({ origin: 'AUDIT', type: 'warning', msg: `[SPIDER] Aucun lien de connexion détecté sur la page d'accueil. Essai des routes standards.` });
        try {
            uniqueLinks.push(new URL('/login', targetBase).toString());
            uniqueLinks.push(new URL('/user/login', targetBase).toString());
            uniqueLinks.push(new URL('/account/login', targetBase).toString());
        } catch(e) {}
    }
    ...
    for (const link of uniqueLinks) {
       ...
       for (const form of forms) {
           ...
           if (emailFilled || passwordFilled) {
               // Perte de focus pour déclencher la validation front-end (blur event) très courante sur React/Vue
               ...
               if (submitBtn) {
                   ...
                   // Double sécurité : on appuie aussi sur Entrée au cas où le clic n'aurait pas été enregistré
                   await page.keyboard.press('Enter').catch(() => {});
               } else {
                   broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[SPIDER] Pas de bouton submit, force form.submit() et touche Entrée` });
