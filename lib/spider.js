...
export async function spiderTarget(targetBase, credentials = {}) {
  ...
  try {
    ...
    for (const link of uniqueLinks) {
       ...
       // Remplissage Humain avec page.$$() et elementHandle.type()
       ...
       for (const form of forms) {
           ...
           for (const input of inputs) {
               ...
               if (type === 'email' || name.includes('email') || name.includes('user')) {
                   await input.click({ delay: Math.random() * 50 + 50 });
                   await input.type(emailToUse, { delay: Math.random() * 50 + 30 });
                   emailFilled = true;
               } else if (type === 'password' || name.includes('pass')) {
                   await input.click({ delay: Math.random() * 50 + 50 });
                   await input.type(passToUse, { delay: Math.random() * 50 + 30 });
               } else if ((type === 'number' || type === 'tel' || name.includes('tel') || name.includes('phone')) && !emailFilled) {
                   await input.click({ delay: Math.random() * 50 + 50 });
                   await input.type('690000000', { delay: Math.random() * 50 + 30 });
               } else if (type === 'text') {
                   await input.click({ delay: Math.random() * 50 + 50 });
                   await input.type(emailToUse, { delay: Math.random() * 50 + 30 });
               }
           ...
           if (submitBtn) {
               ...
               await submitBtn.click({ delay: Math.random() * 50 + 50 });
           } else {
           ...
           if (otpCode) {
               ...
               for (const input of inputs) {
                   ...
                   if (type === 'text' || type === 'number' || name.includes('otp') || name.includes('code')) {
                       ...
                       await input.click({ delay: Math.random() * 50 + 50 });
                       await input.type(otpCode, { delay: Math.random() * 100 + 100 });
                       
                       const form = await input.evaluateHandle(el => el.closest('form'));
                       const isForm = await form.evaluate(f => f !== null);
