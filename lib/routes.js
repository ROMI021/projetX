...
export function makeHandleApi(context) {
    return async function handleApi(req, res, url) {
        ...
        // ---- OTP Submission (BOLA-Shield UI -> Spider) ----
        if (url.pathname === '/api/otp' && req.method === 'POST') {
            const body = await context.readBody(req).catch(() => ({}));
            if (!body.code) return context.sendJSON(res, 400, { error: 'Code OTP requis' });
            context.store.pendingOTP = body.code;
            context.broadcastEvent({ origin: 'SHIELD', type: 'info', msg: `[OTP] Code reçu depuis l'interface (${body.code}), transmission au navigateur fantôme...` });
            return context.sendJSON(res, 200, { success: true, message: 'OTP reçu avec succès' });
        }
        ...
        if (url.pathname === '/api/v1/scanner/audit' && req.method === 'POST') {
            context.store.metrics.dataLeaked = 0;
            context.store.metrics.blockedAttacks = 0;
            context.store.metrics.financialSavings = 0;
            context.store.lastAudit = null;
            context.saveStore();
            ...
            if (!target || (!skipRegistration && (!reg || !login))) {
                ...
                const payload = context.passiveAuditPayload(targetBase, discovery, noAuth ? 'Aucune route login/inscription publique detectee' : 'Configuration audit incomplete', {
                    target,
                ...
                context.store.lastAudit = payload;
                context.saveStore();
                return context.sendJSON(res, 200, payload);
            }
            ...
            try {
                ...
                let loginA = { data: null, attempts: [] };
                let loginB = { data: null, attempts: [] };
                ...
                if (!skipRegistration) {
                    ...
                    if (!regA.ok) {
                        ...
                        const payload = context.passiveAuditPayload(targetBase, discovery, `Inscription utilisateur A refusée par la cible (${regA.status})`, {
                            status: regA.status,
                            response: regA.data,
                    ...
                    if (!regB.ok) {
                        ...
                        const payload = context.passiveAuditPayload(targetBase, discovery, `Inscription utilisateur B refusée par la cible (${regB.status})`, {
                            status: regB.status,
                            response: regB.data,
                    ...
                    if (objectId) context.broadcastEvent({ origin: 'AUDIT', type: 'info', msg: `[ID EXTRAIT] Objet privé détecté depuis l'inscription : ${objectId}` });
                    if (tokenA) context.broadcastEvent({ origin: 'AUDIT', type: 'success', msg: `[TOKEN] Jeton d'accès User A généré avec succès.` });
                    ...
                    loginA = await context.postAuthCandidate(context.joinTargetUrl(targetBase, login), userA, 'login A', loginPayloadsA);

                    await context.delayMs(Math.random() * 800 + 400);

                    loginB = await context.postAuthCandidate(context.joinTargetUrl(targetBase, login), userB, 'login B', loginPayloadsB);
                ...
                if (skipRegistration && !hasManualSession && login) {
                    ...
                    if (!loginA.ok) {
                        ...
                        // FALLBACK: Heavy DAST Engine (Contournement WAF)
                        if (loginA.status >= 400 && context.PUPPETEER_ENABLED) {
                            context.broadcastEvent({ origin: 'AUDIT', type: 'warning', msg: `[WAF DETECTED] Protection stricte détectée. Bascule automatique sur le moteur navigateur lourd...` });
                            const credentials = { userA, userB };
                            const heavyResult = await context.runHeavyAudit(targetBase, discovery.routes, credentials, target);
                            
                            if (heavyResult) {
                                const payload = context.passiveAuditPayload(targetBase, discovery, `Audit finalisé via le moteur navigateur de secours (WAF Bypass)`, {
                                    heavyEngineUsed: true,
                                    bolaConfirmed: heavyResult.bola,
                                    brokenAuthConfirmed: heavyResult.brokenAuth,
                                    httpStatus: heavyResult
                                });
                                payload.objectId = heavyResult.discoveredObjectId || (target && target.includes(':id') ? null : target) || 'Cible-Dynamique';
                                payload.isBOLA = heavyResult.bola;
                                payload.crossStatus = heavyResult.statusB;
                                payload.exfiltrationDump = heavyResult.exfiltrationDump;
                                context.store.lastAudit = payload;
                                context.saveStore();
                                return context.sendJSON(res, 200, payload);
                            }
                        ...
                        const payload = context.passiveAuditPayload(targetBase, discovery, `Connexion directe User A refusée par la cible (${loginA.status})`, {
                    ...
                    if (!loginB.ok) {
                        ...
                        const payload = context.passiveAuditPayload(targetBase, discovery, `Connexion directe User B refusée par la cible (${loginB.status})`, {
                ...
                if (!objectId) {
                    ...
                    [regA?.data, regB?.data, loginA?.data, loginB?.data].forEach(payload => {
                        context.extractPossibleObjectIds(payload).forEach(id => candidateIds.add(id));
                    });
                    
                    if (candidateIds.size === 0 && loginA?.data?.user?.id) candidateIds.add(loginA.data.user.id);
                    if (candidateIds.size === 0 && loginB?.data?.user?.id) candidateIds.add(loginB.data.user.id);
                    if (candidateIds.size === 0 && typeof userA.id === 'string') candidateIds.add(userA.id);
                    if (candidateIds.size === 0 && typeof userB.id === 'string') candidateIds.add(userB.id);
                ...
                let exfiltrationDump = null;
                if (isBOLA) {
                    context.store.metrics.dataLeaked += 1;
                    context.broadcastEvent({ origin: 'ALERT', type: 'alert', msg: `[FUITE BOLA] Audit proxy confirme une fuite sur ${targetUrl}.` });
                    
                    // Lancement du module d'exfiltration via le moteur natif (wrapper HTTP)
                    const executeFetchInPageWrapper = async (pageContext, url, method, body) => {
                        const customHeaders = context.authHeadersForSession({ token: tokenB, cookie: cookieB, scheme: authScheme, referer: targetUrl });
                        if (body && typeof body === 'string') {
                            customHeaders['Content-Type'] = 'application/json';
                        }
                        const res = await context.fetchJSONRemote(url, {
                            method,
                    ...
                    try {
                        exfiltrationDump = await runExfiltrationAndSabotage(null, targetBase, targetUrl, objectId, context.broadcastEvent, executeFetchInPageWrapper);
                        auditRecord.exfiltrationDump = exfiltrationDump;
                    } catch (exfErr) {
                        context.broadcastEvent({ origin: 'AUDIT', type: 'error', msg: `Erreur durant l'exfiltration : ${exfErr.message}` });
                    }
